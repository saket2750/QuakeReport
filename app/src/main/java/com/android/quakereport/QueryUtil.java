...
public final class QueryUtil {
     ...
     */
    public static ArrayList<QuakeReport> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray featuresArray = sampleResponse.getJSONArray("features");
            for(int i =0;i<featuresArray.length();i++){
                JSONObject currIndex = featuresArray.getJSONObject(i);
                JSONObject obj = (JSONObject) currIndex.get("properties");
                String magnitude = String.valueOf(obj.getDouble("mag"));
                String state = (String) obj.getString("place");
                long time = obj.getLong("time");

                earthquakes.add(new QuakeReport(Float.parseFloat(magnitude),state,time));
            }
